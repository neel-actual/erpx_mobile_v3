...
export class EventService {
	...
	async getListing(refresh = false) {
		if (this.listing.length === 0 || refresh) {
			let memberProfile = await this.member.getProfile();

			return this.http
				.get(
					this.config.get_api_url(
						'/api/method/erpx_prulia.prulia_events.doctype.prulia_event.prulia_event.get_event_list'
					),
					{
				...
				.then(res => {
					...
					for (let event of this.listing) {
						if (!event['event_image'] || event['event_image'] === null) {
							...
							event['event_image'] =
								this.config.get_service_endpoint(true) + event['event_image'];
						}
						event['start_date_time'] = new Date(
							event['start_date_time'].replace(' ', 'T') + '+08:00'
						);
						event['end_date_time'] = new Date(
							event['end_date_time'].replace(' ', 'T') + '+08:00'
						);
					}
