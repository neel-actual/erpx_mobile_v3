			...
			return this.http
				.get(
					this.config.get_api_url(
						'/api/method/erpx_prulia.prulia_trainings.doctype.prulia_training.prulia_training.get_training_list'
					),
					{
				...
				.then(res => {
					...
					for (let event of this.listing) {
						if (!event['training_image'] || event['training_image'] === null) {
							...
							event['training_image'] =
								this.config.get_service_endpoint(true) + event['training_image'];
						}
