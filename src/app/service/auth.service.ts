...
export class AuthService {
	...
	login(usr, pwd) {
		...
		return this.http
			.post(this.config.get_api_url('/api/method/login'), credentials)
			.then(data => {
	...
	updatePassword(old_password, new_password) {
		...
		return this.http.post(
			this.config.get_api_url('/api/method/frappe.core.doctype.user.user.update_password'),
			data
		);
	}
	...
	set_sid_cookie() {
		...
		document.cookie =
			'sid=' + localStorage.getItem('session_id') + '; expires=Fri, 31 Dec 9999 23:59:59 GMT';
	}
