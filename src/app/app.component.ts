...
export class AppComponent {
	...
	async syncUpdates() {
		if (window && window['codePush']) {
			await window['codePush'].sync();
		}
	}
	...
	logoutUser() {
		window['plugins'] &&
			...
			window['plugins'].OneSignal.setSubscription(false); //deregister push notification
		this.router.navigate(['login'], {
			replaceUrl: true,
		});
	}

	async presentToast(opts: { message: string; duration: number; color: string }) {
		if (opts.message) {
					...
					filters.forEach(filter => {
						if (
							...
							news[filter]
								...
								.indexOf(memberProfile[filter]) === -1
						) {
							ret = false;
						}
					});
	...
	registerPushNoti() {
		...
		setTimeout(() => {
			try {
				...
				this.oneSignal
					.startInit(environment.ONESIGNAL_APP_ID)
					.iOSSettings({
